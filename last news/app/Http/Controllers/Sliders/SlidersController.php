...
class SlidersController extends Controller
{
    ...
    public function postSliders(Request $request)
    {
        if (isset($request->delete)) {
            try {
                ...
                return response(['status' => 'success', 'title' => 'Başarılı', 'content' => 'Reklam Silindi']);
            } catch (\Exception $e) {
                return response(['status' => 'error', 'title' => 'Başarısız', 'content' => 'Reklam Silinemedi']);
            }
        } elseif (isset($request->slider_status)) {
            try {
                Sliders::where('id', $request->id)->update($request->all());
                return response(['status' => 'success', 'title' => 'Başarılı', 'content' => 'Reklam Durumu Değiştirildi']);
            } catch (\Exception $e) {
                return response(['status' => 'error', 'title' => 'Başarısız', 'content' => 'Reklam Durumu Değiştirilemedi']);
            }
        }
    }
    ...
    public function postSlidersAdd(Request $request)
    {
        try {
            ...
            return response(['status' => 'success', 'title' => 'Başarılı', 'content' => 'Reklam Eklendi']);
        } catch (\Exception $e) {
            return response(['status' => 'error', 'title' => 'Başarısız', 'content' => 'Reklam Eklenemedi']);
        }
    }
    ...
    public function postSlidersEdit(Request $request,$sliderId)
    {
        try {
            ...
            return response(['status' => 'success', 'title' => 'Başarılı', 'content' => 'Reklam Güncellendi ']);
        } catch (\Exception $e) {
            return response(['status' => 'error', 'title' => 'Başarısız', 'content' => 'Reklam Güncellenemedi']);
        }
    }
}
